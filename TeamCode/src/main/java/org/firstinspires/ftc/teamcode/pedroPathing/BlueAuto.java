...
public class BlueAuto extends OpMode {
    ...
    // Robot hardware (from your teleop)
    ...
    private static final double SPINUP_TIME = 4.5; // seconds
    private static final double TARGET_VOLTAGE = 13.8; // Target voltage for compensation

    // Poses from your trajectory (adjusted for robot center offset)
    private final Pose startPose = new Pose(35, 130, Math.toRadians(90));
    private final Pose path1End = new Pose(50, 85, Math.toRadians(135));
    private final Pose path2End = new Pose(55, 70, Math.toRadians(180));
    private final Pose path3End = new Pose(37, 70, Math.toRadians(180));
    private final Pose exitPath1 = new Pose(50.82, 68.24, Math.toRadians(180));

    // Paths
    private Path firstPath, secondPath, thirdPath, returnPath, exitPath;
    ...
    public void init() {
        ...
        // Set directions (from your teleop)
    ...
    public void buildPaths() {
        ...
        // Third path: intermediate position to final position (VERY SLOW with intake running)
        ...
        // Return path: go back to shooting position
        ...
        // Exit path: leave triangle and go to pose2
        exitPath = new Path(new BezierLine(path1End, exitPath1));
        exitPath.setLinearHeadingInterpolation(path1End.getHeading(), exitPath1.getHeading());
    }
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 3:
                ...
                break;
            case 4:
                // Wait 1 second, then shoot second ball
            ...
            case 5:
                ...
                break;
            case 6:
                // Wait 1 second, then shoot third ball
                if (pathTimer.getElapsedTimeSeconds() >= 2.0) {
                    lifter.setPosition(up);
                    pathTimer.resetTimer();
                    setPathState(7);
            ...
            case 14:
                ...
                break;
            case 15:
                // Wait 1 second, then shoot second ball (round 2)
            ...
            case 16:
                ...
                break;
            case 17:
                // Wait 1 second, then shoot third ball (round 2)
                if (pathTimer.getElapsedTimeSeconds() >= 1.5) {
                    lifter.setPosition(up);
                    pathTimer.resetTimer();
                    setPathState(18);
            ...
            case 18:
                ...
                if (pathTimer.getElapsedTimeSeconds() >= 0.1) {
                    lifter.setPosition(down);
                    flywheel.setPower(0); // Stop flywheel
                    follower.followPath(exitPath, true); // Exit to pose2
                    setPathState(19);
                }
                break;
            case 19:
                // Wait for exit path to complete
                if (!follower.isBusy()) {
                    // All done - robot now at pose2 (outside triangle)
                    setPathState(-1);
                }
                break;
        }
